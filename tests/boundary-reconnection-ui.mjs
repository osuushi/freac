...
try {
  ...
  await roundReconnection(page, name, app);
  await singleEdgeReconnection(page, name, app);
  for (const kind of ["hole", "pocket", "boss"]) await featureReconnection(page, name, app, kind);
} catch (error) {
  await page?.screenshot({ path: `.cache/sketch-review/${name}-reconnection-failure.png` });
  console.log(
    await page?.evaluate(() => ({
