...
try {
  ...
  for (const kind of ["hole", "pocket", "boss"]) await deleteFeatureRoute(page, name, app, kind);
  await deleteEdgeRoute(page, name);
  await cleanupRoute(page, `${name}-delete-regression`, app);
} catch (error) {
  await page?.screenshot({ path: `.cache/sketch-review/${name}-deletion-failure.png` });
  console.log(
    await page?.evaluate(() => ({
