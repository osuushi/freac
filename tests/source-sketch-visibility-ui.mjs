...
import {
  sourceSketchPreviewUndoRoute,
  sourceSketchVisibilityRoute,
} from "./ui-source-sketch-visibility.mjs";
...
try {
  ...
  await sourceSketchVisibilityRoute(page, name);
  await sourceSketchPreviewUndoRoute(page, name);
} catch (error) {
  await page?.screenshot({
    path: `.cache/sketch-review/${name}-source-sketch-visibility-failure.png`,
  });
  console.log(
    await page?.evaluate(() => ({
