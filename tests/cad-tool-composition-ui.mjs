...
async function route(page, name) {
  ...
  await page.getByRole("textbox", { name: "Scale factor" }).fill("2");
  await inspect(page);
  await page.getByRole("button", { name: "Accept scale" }).click();
  ...
  await page.getByRole("button", { name: "Select Body 1", exact: true }).click();
  await chooseTool(page, "scale", "scale");
  await page.getByRole("textbox", { name: "Scale factor" }).fill("0.5");
  await inspect(page);
  await page.getByRole("button", { name: "Accept scale" }).click();
