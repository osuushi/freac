...
export async function moveCopiedJunction(page, source, result) {
  ...
  await inspectPointChoices(page, -12, -8);
  await page.getByRole("button", { name: "Point 1", exact: true }).click();
  await page.keyboard.press("Escape");
  await drag(page, [-12, -8], [-14, -10], ["Shift"]);
  ...
  for (const old of result.curves.slice(4)) {
    ...
    for (const end of ["a", "b"]) {
      const corner = old[end].x === -12 && old[end].y === -8;
      pointEquals(current[end], corner ? [-14, -10] : [old[end].x, old[end].y]);
      if (corner) count++;
    }
  }
