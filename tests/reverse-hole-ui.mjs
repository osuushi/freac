...
import { reverseHoleRoute } from "./ui-reverse-hole.mjs";
...
try {
  ...
  await reverseHoleRoute(page, name);
} catch (error) {
  await page?.screenshot({ path: `.cache/sketch-review/${name}-reverse-hole-failure.png` });
  console.log(
    await page?.evaluate(() => ({
