...
export async function rectangleBowRoute(page, name) {
  for (const plane of ["XY", "XZ", "YZ"]) {
    ...
    pointEquals(arc.a, [-10, -6]);
    pointEquals(arc.b, [10, -6]);
    ...
    await page.keyboard.press("Enter");
    pointEquals((await inspect(page)).document.sketches[0].curves[0].a, [-10, -6]);
    await chooseTool(page, "select", "select");
    await drag(page, [-10, -6], [-12, -6], ["Shift"]);
    const edited = (await inspect(page)).document.sketches[0];
    pointEquals(edited.curves[0].a, [-12, -6]);
    pointEquals(edited.curves[3].b, [-12, -6]);
    await chooseTool(page, "undo", "undo");
    await chooseTool(page, "undo", "undo");
    await chooseTool(page, "undo", "undo");
    assert.deepEqual((await inspect(page)).document, original);
